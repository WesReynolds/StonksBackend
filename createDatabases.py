...
def create_transactions():
    ...
    #cur.execute("CREATE TABLE Transactions(UserId int REFERENCES Users (id), ticker varchar(10), volume float(0), action varchar(5), price float(0), time DATETIME(0))")
    time = datetime.datetime.now()
    cur.execute("INSERT INTO Transactions Values (1, 'AAPL', 10, 'BUY', 134.85, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (1, 'AAPL', 2, 'SELL', 134.85, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (3, 'AAPL', 10, 'BUY', 134.85, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (7, 'AAPL', 7, 'SELL', 134.85, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (7, 'GME', 12, 'BUY', 52.40, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (7, 'GME', 12, 'SELL', 4.25, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (2, 'AAPL', 10, 'BUY', 134.85, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (2, 'AAPL', 10, 'BUY', 134.85, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (10, 'AAPL', 10, 'BUY', 134.85, '%s')" % time)
    cur.execute("INSERT INTO Transactions Values (10, 'AAPL', 10, 'BUY', 134.85, '%s')" % time)
    # Commit change 
    cnx.commit()
    # Close connections 
